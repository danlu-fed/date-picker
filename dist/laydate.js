...
// 切换class
Dates.swapClass = function(ele,nClass,oClass){
    var c = ele.className;
    if (c != null && c.indexOf(oClass) > -1) {
        Dates.removeClass(ele,oClass);
        Dates.addClass(ele,nClass);
    }else{
        Dates.addClass(ele,nClass);
    }
};
...
/*
 * 时分秒有效性检测
 * @param {Number} times -> 选中的时间
 * @param {Number} index ["0":"时","1":"分","2":"秒"]
 * @return {Number}
*/
Dates.timeVoid = function(times, index){
    /*
    *       源代码只校验了年月日， 16年12月2日增加时，分的判断
    */
    // 时间可行域左端点
    if(Dates.ymd[1]+1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]){
        // 半小时
        var minh = Dates.mins[3];
        if (Dates.halfhours() && Dates.mins[4] > 30) {
          minh = Number(minh) + 1;
        }
        if(index === 0 && (times < minh)){
            return 1;
        } else if(index === 1 && Dates.hmsin[0].value <= Dates.mins[3] && times < Dates.mins[4] ){
            return 1;
        } else if(index === 2 && Dates.hmsin[0].value <= Dates.mins[3] && Dates.hmsin[1].value <= Dates.mins[4] && times < Dates.mins[5] ){
            return 1;
        }
    // 时间可行域左端点
        ...
        if(index === 0 && times > Dates.maxs[3]){
            return 1;
        } else if(index === 1 && Dates.hmsin[0].value >= Dates.maxs[3]  && times > Dates.maxs[4]){
            return 1;
        } else if(index === 2 && Dates.hmsin[0].value >= Dates.maxs[3] && Dates.hmsin[1].value >= Dates.maxs[4] && times > Dates.maxs[5]){
            return 1;
        }
    }
...
Dates.viewDate = function(Y, M, D){
    ...
    Dates.each(as.tds, function(i, elem){
        ...
        if(i < log.FDay){
            DD = i + log.PDay;
            elem.innerHTML = '<span class="lay-day">' + DD + '</span>';
    ...
    Dates.each(new Array(3), function(i){
        var minhms = Dates.mins[i+3]|0;
        if (Dates.halfhours()) {
          // 确定时
          if (i === 0 && Dates.mins[i+4] > 30) {
            minhms = minhms + 1;
          // 确定分
          } else if (i === 1) {
            if (minhms <= 30) {
        ...
        Dates.hmsin[i].value = Dates.digit(Dates.timeVoid(log.times[i], i) ? minhms|0 : log.times[i]|0);
    });
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        Dates.swapClass(Dates.box,'lay-show','lay-hide');
    }
...
Dates.close = function(){
    Dates.reshow();
    Dates.swapClass(Dates.query('#'+ as[0]),'lay-hide','lay-show');
...
Dates.events = function(){
    ...
    Dates.msg = function(i, title){
        ...
        if(typeof i === 'string'){
            ...
            /*
             * 改动说明去掉了检查缓存的一层判断为了取ele中"harfmin"的值，选择性渲染
            */
            ...
            */
            if (Dates.elem.getAttribute('halfhours') === "true") {
                if (i === 0) {
                    ...
                    Dates.each(new Array(2), function(i){
                        str += '<span>'+ i*30 +'</span>';
                    });
                }else if (i === 2) {
                    Dates.each(new Array(1), function(i){
                        str += '<span>'+ 0 +'</span>';
                    });
                }
            } else {
    ...
    Dates.on(doc, 'mouseup', function(){
        var box = S('#'+as[0]);
        if(box && !Dates.hasClass(box,'lay-hide')){
